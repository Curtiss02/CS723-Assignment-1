...
#define LCD_TASK_1_PRIORITY 	7
#define LCD_TASK_2_PRIORITY 	6
...
void lcd_task_1(void *pvParameters){
	while(1){
...
void lcd_task_2(void *pvParameters){
	while(1){
		xSemaphoreTake(shared_lcd_sem, portMAX_DELAY);
		FILE* lcd;
		lcd = fopen(CHARACTER_LCD_NAME, "w");
...
int initCreateTasks(void)
{
	...
	xTaskCreate(lcd_task_1, "lcd_task_1", TASK_STACKSIZE, NULL, LCD_TASK_1_PRIORITY, NULL);
	xTaskCreate(lcd_task_2, "lcd_task_2", TASK_STACKSIZE, NULL, LCD_TASK_2_PRIORITY, NULL);
